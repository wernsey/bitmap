 ...
 * 
 * __FIXME__: _The bezier works kinda-sorta for specific values of the control points, but
 * it is unreliable, so I'd replace it with something more robust at a later stage._
 */
void bm_bezier3(Bitmap *b, int x0, int y0, int x1, int y1, int x2, int y2);
 ...
 *   - [] and that `x0,y0` is inclusive and `x1,y1` is exclusive.
 ...
 * - [] `bm_bezier3()` needs fixing.
 * - [x] There are lots of functions (especially ones related to color) that can be 
 *      removed to simplify the API. Just search this header for "__DEPRECATED__".
 * - [x] Occurances of _"colour"_ should be replaced with _"color"_ - for internal consistency
 *      and consistency with other APIs.
 * - [] I've added a precompiler definition IGNORE_ALPHA which, if set, should cause all color
