...
int bmf_init() {
	...
	if(error) {
		/*fprintf(stderr, "An error occured during FreeType init (%d).\n", error);*/
		return 0;
	}
	return 1;
}
...
BmFont *bmf_load_font(const char *file) {
	...
	if(error == FT_Err_Unknown_File_Format) {
		...
		/*fprintf(stderr, "An error occured during loading the font face (%d).\n", error);*/
		free(bmf);
		return NULL;
	}
...
static FT_Face face_from_rwops(SDL_RWops *rw) {
	...
	if(position < 0) {
		/* fprintf(outfile, "can't seek in RW (%d)\n", position); */
		return NULL;
	}
	...
	if(error) {
		/*fprintf(outfile, "error in face_from_rwops %d\n", error);*/
		return NULL;
	} else {
		return face;
	}
